...
public class OthelloBoard {
	...
	private char checkForPattern(int row, int col, int drow, int dcol) {
		...
		while (true) {
			...
			if (seq == EMPTY) {
				seq = this.board[row][col];
			}
			else if (seq != this.board[row][col]) {
				return this.board[row][col];
			}	
			row = row + drow;
			col = col + dcol;
	...
	private int flip(int row, int col, int drow, int dcol, char player) {
		...
		else if (this.checkForPattern(row, col, drow, dcol) == OthelloBoard.otherPlayer(player)) {
			return c;
		}
		else {
...
//		char old = this.getToken(row, col);
		boolean flag = false;
		int r = -1;
		int c = -1;
		while(r < 2) {
			while(c < 2) {
				if (!(r == 0 && c == 0)) {
					int v = this.flip(row + r, col + c, r, c, player);
					if (v > 0) {
						flag = true;
					}
				}	
